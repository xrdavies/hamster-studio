...
当前轮尚未输出内容时，对话模型临时错误最多自动重试两次（约 2 秒、5 秒并加入随机延迟）。检查点保留待执行模型消息和已完成工具结果。部分回复中断后可点击“继续此步骤”；图片失败和用户取消不重放。网页读取在原 30 秒预算内重试。Rig 当前错误接口未暴露 Retry-After 响应头，模型列表获取暂仍需手动重试。
